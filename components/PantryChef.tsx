...
const PantryChef: React.FC<PantryChefProps> = ({ user, onUpdateUser }) => {
  ...
  return (
    <div className="flex flex-col h-full animate-enter">
      {/* کادر ثابت دستیار آشپز */}
      <div className="sticky top-0 z-[900] bg-[#f8fafc]/95 backdrop-blur-md px-4 py-3 sm:py-6 sm:px-10">
          <div className="metallic-navy rounded-[1.75rem] sm:rounded-[3.5rem] p-4 sm:p-8 shadow-2xl overflow-hidden relative border border-white/5 max-w-7xl mx-auto">
            <div className="flex flex-col lg:flex-row items-center justify-between gap-3 sm:gap-4 relative z-10">
              <div className="flex items-center gap-3 sm:gap-4 text-right flex-row w-full lg:w-auto">
                ...
                <div>
                  <h2 className="text-sm sm:text-2xl font-black text-white leading-none">دستیار آشپز برتر</h2>
                  <p className="text-teal-400 text-[8px] sm:text-xs font-bold mt-1 opacity-80 uppercase tracking-widest">Smart Pantry Discovery</p>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full lg:w-auto">
                <div className="relative flex-grow">
                   <input type="text" placeholder="جستجوی مواد غذایی..." value={pantrySearchTerm} onChange={(e) => setPantrySearchTerm(e.target.value)} className="w-full sm:w-80 px-10 sm:px-12 py-2.5 sm:py-4 bg-white/10 border border-white/20 rounded-xl sm:rounded-2xl text-white font-bold outline-none focus:border-teal-500 transition-all placeholder:text-slate-500 text-xs sm:text-sm" />
                   <Search className="absolute right-3 sm:right-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} sm:size={20} />
                </div>
                <button onClick={findRecipes} className="w-full sm:w-auto px-6 py-2.5 sm:px-10 sm:py-4 bg-teal-500 hover:bg-teal-400 text-navy-900 rounded-xl sm:rounded-2xl font-black shadow-lg flex items-center justify-center gap-2 sm:gap-3 transition-all active:scale-95 disabled:opacity-40 text-xs sm:text-sm" disabled={selectedItems.length === 0 || isSearching}>
      ...
      <div className="flex-grow overflow-y-auto px-4 sm:px-10 pb-20 no-scrollbar">
          ...
          <div className="max-w-7xl mx-auto py-4 sm:py-6 space-y-6 sm:space-y-10">
              ...
              {/* Results */}
              {rawResults && (
                <div className="space-y-6 sm:space-y-10 animate-enter pt-6 sm:pt-10 border-t border-slate-100">
