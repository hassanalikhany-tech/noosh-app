...
export const NotificationService = {
  /**
   * Create a new notification for a target group or specific user
   */
  createNotification: async (title: string, message: string, targetGroup: 'all' | 'active' | 'visitors' | 'specific', adminId: string, targetUid?: string) => {
  ...
  /**
   * Get notifications for a user based on their role, activity and UID
   */
  getUserNotifications: async (userId: string, role: string, isActive: boolean): Promise<Notification[]> => {
    ...
    // Filter by target group and target UID
    return allNotifs.filter(n => {
      if (n.target_group === 'specific') {
        return n.target_uid === userId;
      }
  ...
  /**
   * Log that a notification was delivered/seen (simplified with local storage usually)
   */
